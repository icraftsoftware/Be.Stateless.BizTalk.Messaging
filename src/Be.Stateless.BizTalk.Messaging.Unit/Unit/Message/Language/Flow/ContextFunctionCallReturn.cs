...
{
	internal class ContextFunctionCallReturn<TMock, TResult> : IReturnsThrows<TMock, TResult>
		where TMock : class, IBaseMessage
	{
		...
		{
			_contextFunctionCallReturnImplementation = contextFunctionCallReturnImplementation ?? throw new ArgumentNullException(nameof(contextFunctionCallReturnImplementation));
		}
