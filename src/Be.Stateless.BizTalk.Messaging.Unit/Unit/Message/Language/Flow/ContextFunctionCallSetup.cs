...
namespace Be.Stateless.BizTalk.Unit.Message.Language.Flow
{
		...
		where TMock : class, IBaseMessage
	{
		...
		{
			_contextFunctionCallSetupImplementation = contextFunctionCallSetupImplementation ?? throw new ArgumentNullException(nameof(contextFunctionCallSetupImplementation));
		}
		...
		{
			return _contextFunctionCallSetupImplementation.Throws(exceptionFunction);
		}
		...
		{
			return new ContextFunctionCallReturn<TMock, TResult>(_contextFunctionCallSetupImplementation.Callback(callback));
		}
