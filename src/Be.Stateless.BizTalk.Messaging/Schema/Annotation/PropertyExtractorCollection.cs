...
namespace Be.Stateless.BizTalk.Schema.Annotation
{
	...
	public class PropertyExtractorCollection : IEnumerable<PropertyExtractor>, IXmlSerializable
	{
		...
		public static PropertyExtractorCollection Empty => EmptyPropertyExtractorCollection.Instance;

		public PropertyExtractorCollection() : this(default(ExtractorPrecedence)) { }

		public PropertyExtractorCollection(IEnumerable<PropertyExtractor> extractors) : this(default, extractors) { }
		...
		public virtual void ReadXml(XmlReader reader)
		{
			try
			{
				ReadXmlProperties(reader ?? throw new ArgumentNullException(nameof(reader)));
				ValidateExtractors();
			}
		...
		{
			if (Extractors.Any()) WriteXmlProperties(writer ?? throw new ArgumentNullException(nameof(writer)));
		}

		#endregion

		/// <summary>
		/// Precedence of the <see cref="PropertyExtractorCollection"/> to be used should extractors declared by XML schema annotations be merged to extractors
		...
		{
			if (pipelineExtractors == null) throw new ArgumentNullException(nameof(pipelineExtractors));
			...
			while (reader.NodeType == XmlNodeType.Element)
			{
				...
				var mode = reader.HasAttribute("mode")
					...
					: reader.HasAttribute("promoted")
						? Convert.ToBoolean(reader.GetAttribute("promoted"), CultureInfo.InvariantCulture)
		...
		private void WriteXmlProperties(XmlWriter writer)
		{
			writer.WriteStartElement("s0", "Properties", SchemaAnnotation.NAMESPACE);
			{
				...
				{
					// see https://docs.microsoft.com/en-us/dotnet/api/system.xml.xmltextwriter
		...
		[Conditional("DEBUG")]
		[SuppressMessage("ReSharper", "PossibleMultipleEnumeration", Justification = "Any does not perform a thorough enumeration.")]
		private void ValidateExtractors()
		{
			...
			if (invalidExtractors.Any())
				throw new XmlException(
					"The following properties are not associated with the target namespace URI of some property schema: "
			...
			if (duplicateExtractors.Any())
				throw new XmlException(
					$"The following properties are declared multiple times: [{string.Join("], [", duplicateExtractors.Select(p => p.ToString()).ToArray())}].");
		}
